...
- Reward: R1' arc-length over REBUILT corrected-aero reference line (reference_line_vq1.json is drag-infeasible + 170° inverted → MUST rebuild = C4); T4 finish-time KEPT; R4' fixed relaxed cone L0 default (tilt_free_rad is scalar; confidence-gated cone = portfolio ablation); **🆕 R5' camera-pointing / gate-in-FoV reward = NEW TOP LEVER (2-AXIS: azimuth + ELEVATION)** — keep active gate centre in-image. ELEVATION CO-BINDS (2026-06-14 FIX-SURROGATE finding): after de-crab, gate is in horizontal FoV 100% but vertical FoV only ~10% (|el|~45° vs VFoV 29° at +20° mount); centering through the 18–28 m window = ×8.5 in-window fix-density gain (0.099→0.84). Decrab (yaw — translationally ~free) is NECESSARY but NOT SUFFICIENT; elevation couples to PITCH = the accel knob. 🚩 **MOUNT FIXED at 20° by spec §3.8 (confirmed 2026-06-14) — mount-uptilt lever DEAD; the "mount-uptilt sweep / cheap mount knob" is OFF THE TABLE; elevation fix-rate is POLICY-only via pitch/pointing (no free knob).** L3 MEASURED gate-4 fix-rate = **1.4% of frames** (gate-4 worst; pooled ~7%): crab p50 66°, at >60° crab (51% of frames) gate-4-in-FoV 5% / 0 fixes; ALL 81 fixes from the crab-30–45° gate-3-approach window. This is the binding blocker. Reward the OUTCOME (estimator error vs GT privileged); let pointing + fix timing EMERGE. Do NOT hand-reward a specific yaw setpoint or hand-code a fix-timing schedule. Speed/finish dominant; flat (no gate-proximity weighting — optimal fix phase unknown); dense shaping via frozen obs channels (c_inplane, c_along, age_norm) annealed late. **✅ FIX-SURROGATE DONE (infra, branch claude/stupefied-fermi-20126c, NOT merged):** `rl/fix_surrogate.py` analytic NO-RENDER fix model; calibrated to Track-3 (703 green); σ single swappable checkpoint for L3 recalibration; REWARD NOT DECIDED. → [[index-vision-estimator]] §FIX-SURROGATE + §2-AXIS CAMERA-POINTING FINDING
...
- **🆕 INC8 LEVERS — BOTH VERTICAL-AXIS, CO-EQUAL (2026-06-14 δ_map discriminator):** (1) 2-axis camera-pointing / fix-RATE (R5' reward, surrogate-driven — pointing gets fixes); (2) BORESIGHT/attitude-bias estimation / fix-ACCURACY (ε_vert ≈ 0.215 m / ~0.56°, CALIBRATABLE, +L does NOT cancel — boresight makes fixes true). ESKF attitude-bias estimation PROMOTED from secondary to co-equal lever. See §δ_MAP VERTICAL DISCRIMINATOR in [[index-vision-estimator]].
- **🚩 TERMINAL GATE-LOCK = LOAD-BEARING REFINEMENT of the camera-pointing spec (2026-06-14; margin-closure-envelope 7654e99):** 268-cell sweep + 4-lens adversarial pass shows CLUSTERING is dominant — terminal fix-DROUGHTS break closure at any mean fix-rate. r=0.30 closes IFF fix-rate ≥ ~0.25 WITH camera held through ≥60% of the final ~5 m approach; a high pooled mean is NOT sufficient. **The R5' camera-pointing reward must target TERMINAL GATE-LOCK (hold the gate through the approach), not just a high average gate-in-FoV rate.** Naive fix-diff velocity channel is HARMFUL at sparse fix-rate → keep velocity channel OFF / P2 insurance. GPU latency ceiling ≤ ~50 ms. See §MARGIN-CLOSURE-ENVELOPE in [[index-vision-estimator]].
...
~~P0 bug fixes → C2 estimator~~ ✅ DONE (05ed750, 723 green) → C1 obs → **NEXT = C6 deploy-loop wiring → POC flight → C5 inc8 retrain (LONGEST POLE, ≥5 seeds)** → C6 gauntlet G0–G7 → C6 live L0–L4. C3 margin sim + C4 corrected line = OFF critical path (parallel feeders). NEES: c1's 1.96 = in-plane 2-DOF (mildly OVER-confident), NOT the χ²(3)=3 target — calibrate c_cal on L3.

### Ranked residual risks (top 3) — UPDATED 2026-06-14 by δ_map discriminator
1. Gate-4 p90/p99 margin DOES NOT clear offline for the realistic case-C tail (cold p90@1.4°-bias = 0.338 m — fails at EVERY admissible radius at p90 AND p99). RMS-clears are p90-fails. **BINDING FACTOR = VERTICAL BORESIGHT BIAS ε_vert ≈ 0.215 m (~0.56°, CALIBRATABLE) + EFFECTIVE SYSTEMATIC ATTITUDE/ACCEL BIAS** (not radius — radius reconciliation confirmed). L3 "CLOSE" REFUTED by δ_map discriminator (case b, d7c592e). CANNOT-SETTLE-OFFLINE SURVIVES. Path to close = boresight calibration + ESKF attitude-bias estimation. **HIGH** → morning boresight lock test (static head-on level fix at g2/g4) + ESKF pipeline.
2. Boresight ε_vert ≈ 0.215 m (~0.56°) UNPINNED airtightly (bearing + depth-free + resurvey evidence → case-b high-confidence; airtight = morning ShadowPC static head-on level fix). ESKF attitude-bias state = CO-EQUAL MARGIN LEVER (PROMOTED from secondary by δ_map finding; alongside 2-axis camera-pointing/fix-rate). **HIGH** → morning ShadowPC boresight LOCK test + ESKF attitude-bias estimation pipeline.
