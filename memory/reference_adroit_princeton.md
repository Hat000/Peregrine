...
- 🚩 **TOOLKIT (re-confirmed 2026-07-13, Fengyou pointed me back to the docs after I fumbled) — READ `adroit-connector/README.md` + `CLUSTER.md` BEFORE driving it.** First-class subcommands: `run "cmd"` (1 Duo/cmd — avoid), **`serve`** (1 Duo, then reuse), **`x "cmd"`** (via daemon, no new Duo, **300 s socket timeout**), `shell` (interactive, 1 Duo/session), `upload <local> <remote-file>`, `submit <script>` (bare `sbatch <path>` — **does NOT pass `--export`**, so env-var launches go through `x "sbatch --export=ALL,... <script>"`), **`jobs`** (clean `squeue` — use it, don't hand-roll `-o`), `tb <jobid>`. Helpers: **`pull_file.py <remote> <local>`** (chunked, sha256-verified download — use for .hydra/logs/ckpts), **`watch_job.py <jobid>`** (poll a job to completion in bg). Everything logs to `command_history.log`. 🚩 **KEEP `x` CHEAP + SIMPLE — one op, NO parens/`()`, no `ls -la` on big ckpt dirs, no heavy `grep`**; today a compound `ls -la .../checkpoints + grep` timed out the socket and a `echo ...(vpefwh2)...` broke remote bash on the paren. For anything complex/multiline, ship it as a base64'd script file (per the bullet above) or `upload` a launch script and run THAT — never inline a big `--export` list.
