...
🚩 **BAR CORRECTED 2026-06-19 (Fengyou) — read before the "0.08" verdicts below.** Every "≲0.08" / "NO-GO vs 0.08" / "near-field-estimator pivot" in the bullets below was judged against a WRONG bar: the σ_p0 0.08 "closure bar" was a `margin_envelope.py` DOUBLE-COUNT of the drone (W_EFF=0.75−0.215 chassis, then MARGIN=W_EFF−r → the drone subtracted TWICE; 0.08 = 0.235/3). The SIM is correct (pass = offset < 0.75−r, r∈[0.28,0.38] → real clearance **0.37–0.47 m**); the REAL bar is **σ_p0_lat ≲0.15 (p99 ≲0.45)**; measured σ_p0 0.15–0.20 is **MARGINAL-PASSING, not NO-GO.** The "pivot off RL to a near-field-gate estimator" is **RETRACTED** — RL stays the tool; gate-4 is a REACH/PASS-RATE problem (the gate sits OFF the racing line), not sub-cm centering. The episodic detail below STANDS (mechanisms, bugs, milestones); only the 0.08 GO-verdicts are superseded → see the §inc8-2026-06-19 closing entry.
...
- **S4 IN-TRAINING COURSE-CHECK = PURE POLICY-GAP CONFIRMED (confound-free, Adroit log-read, 2026-06-17).** md5-verified the in-training TB event files for all 3 ws seeds + the S2-seed2 parent; parsed metrics/success_rate + metrics/n_passed_gates (env logs @ peregrine_racing_inc8.py:443). **success_rate ≡ 0.0000 flat across ALL 400 points (steps 0→3990), ALL 4 runs** — never rose above 0. n_passed_gates floored ~0.10-0.14 (no upward trend); survive_rate≡0 (never time out — always miss/crash); miss_rate≈0.98; l_episode≈1.5 s (lap ~8 s → dies at/near gate-0). Pointing genuinely worked in training (pointing_rate max 0.46, terminal_pointing 0.12-0.15, inc8_fix_rate 0.17-0.23) → **the ladder optimized POINTING on a NON-FLYING policy.** 🚩 the **S2-seed2 parent already has success_rate≡0** → the gap is INHERITED across the whole inc8 lineage, NOT introduced by warm-start (or S3). The laptop reach=0/200 was FAITHFUL, not a look-at-reinject artifact. 🚩 **FIX (VQ2-gated): reward architecture — restore through-approach centring / the dropped R1-to-centre (peregrine_racing_inc8.py:329) + ADD course-completion (success_rate) to the inc8 GO at every stage + re-train.** Porting look-at into the eval is worth doing later for a faithful σ_p0 but won't rescue these ckpts (no lap to measure). 🚩 **CRAFT LESSON (→ COMMANDER.md): gate on the END OBJECTIVE (course-completion), never only an intermediate PROXY (pointing) — the proxy-wins masked the objective-failure for the ENTIRE ladder (S0→warm-start).** σ_p0 NO-GO until a policy completes a lap. Report: handoff/inc8-intraining-coursecheck-2026-06-17/REPORT.md.
- **S5 RECENTER FIX BUILT + MERGED (branch p2-inc8-recenter → main 9cecf64, 2026-06-17; NOT launched).** Diagnosis CONFIRMED IN CODE: R1-to-centre (`peregrine_racing.py:364` = `prev_d2g − curr_d2g`, the Euclidean dist-to-gate-CENTER delta — radial gradient has BOTH forward-closing AND lateral-restoring components → centers across the whole approach; inc7 flies 3/3 with it). inc8 ZEROED it (`replace(self.rw, progress=0.0)`) for arc-Γ progress (ALONG-TRACK only, no lateral restoring) + a near-gate term that penalizes ESTIMATOR error (σ_p0) not physical offset → drift off the center line → gate-0 miss → success_rate≡0 lineage-wide. **FIX** = `rl/inc8_reward.through_centering_reward` = R1-to-centre's LATERAL component ISOLATED (cross-track gate-frame in-plane `hypot(y,z)` dist-to-center-line delta, whole approach) — ORTHOGONAL to arc-Γ (along-track) so it adds exactly the missing restoring force with NO forward-progress double-count + NO M-2 corner-cut re-intro; composes with arc-Γ + near-gate σ_p0 centering + look-at; 20-dim obs unchanged. Gated `+env.rw_through_centering=<w>`; =0 → bit-exact byte-id inc8 (torch.equal); 16 reward tests (3 new) + 97 inc8 + OFF==inc7 AST + green_gate GREEN (940). 🚩 **GO-criterion lesson baked in:** `inc8_tb_trace.py` now surfaces metrics/success_rate + n_passed_gates from step 0 + a greppable FLIGHTCHECK line; `peregrine_inc8_recenter.sbatch` = 3 FRESH seeds (NO warm-start — lineage never flew), RW_TC=10.0, ++lookat -3.0/+3.0, gain-warmup 200, per-seed run.dir, AUP, + **auto early-stop after seed-0 if success_rate~0 ∧ n_passed_gates~0** (don't burn seeds 1,2). rw_centering stays OFF (single-variable change; it's the S3-cliff driver — layer it back only AFTER laps complete). Launch: `sbatch --export=ALL,RUNTAG=rc1 .../peregrine_inc8_recenter.sbatch` (Fengyou, Adroit, idle-time). 🚩 **THE GO IS success_rate OFF ZERO** (the first flying inc8), not pointing; σ_p0 only meaningful AFTER laps complete; if RW_TC=10.0 over/under-centers, sweep via RW_TC=. Re-training inc8 (20-dim look-at) = the CASE-C bet; if VQ2 gives position the same centering-fix insight ports to a simpler inc7-recenter. Report: handoff/inc8-recenter-build-2026-06-17/REPORT.md.
- **S5 RECENTER FLEW — FIRST FLYING inc8 (job 3276449, 2026-06-18; all 3 seeds RC=0).** success_rate_max: seed0 0.538 / seed1 0.513 / seed2 0.504; n_passed_gates_max 2.96-3.03; tightly clustered. The SINGLE change (rw_through_centering=10.0 — the restored R1-to-centre cross-track pull) recovered flight from the success_rate≡0 lineage (S2 parent + warm-start all died at gate-0 ~1.5 s) → 🚩 **the dropped through-approach centring WAS the policy-gap root cause, fixed REPRODUCIBLY (3/3 fresh seeds).** Pointing RETAINED but rebalanced down (all pointing metrics >0, far below the pointing-optimized-never-flew lineage = the intended trade: budget → course-completion). 🚩 **σ_p0 NOW MEASURABLE (laps complete + #37 proved the emul obs faithful at Δ≤9.2e-9) → NEXT = offline contact_true_eval / inc8_sigmap0_eval on outputs/train/inc8_recenter_seed{0,1,2}_rc1/** — report reach@gate-4 + σ_p0 std/bias/p90-p99 vs the 0.08 target. HEADROOM: RW_TC=10.0 plateaus ~0.5 / n_passed ~3.0 → a RW_TC sweep (6/14) is the natural flight-headroom follow-up; MonoRace idea#1 (ABSOLUTE cross-track centring, never-cancels) is the σ_p0-TIGHTENING upgrade IF the delta-form under-centres (a delta can be paid back by a late re-centre). Caveat: inc8_tb_trace has no through_centering column (the traced 'centering' = the OFF near-gate term); restore confirmed indirectly (clean precheck + single-var FLEW) — add the column next run. Ops: 4 fix files md5-verified to Adroit; #76 daemon wedge recovered. Report: handoff/inc8-recenter-run-2026-06-17/REPORT.md.
- **S5 σ_p0 EVAL on rc1 = UN-MEASURABLE = EVAL-HARNESS GAP, not the policy (2026-06-18):** reach@gate-4 ~0/200 (seed0 1/200 fluke; finished 0/200), per-gate diag = passes ZERO gates in 56/60, dies at/before gate-0 — CONTRADICTS training (success_rate ~0.50-0.54, n_passed ~3.0 across 2048 randomized starts). ROOT (disambiguated): the eval (contact_true_eval / inc8_sigmap0_eval, --estim-emul) OMITS the look-at primitive training composes onto the action (contact_true_eval.py:307 vs peregrine_racing_inc8.py:226-246) → inc8's executed control is wrong → non-flight. inc7 (no look-at) flies the SAME harness 3/3 = harness is flyable + the omission is inc8-specific. The truth-obs path is unusable for inc8 (builds 17-dim; the 20-dim actor needs the emul confidence-triple → shape error), so --estim-emul is the only valid inc8 obs path and the non-flight lives in it. 🚩 **σ_p0 GO is BLOCKED on the eval-harness look-at PORT** (the spike's + scout's flagged carry-forward, now LOAD-BEARING); a RW_TC sweep is moot until the eval can fly inc8. NEXT = port look-at into contact_true_eval (inc8-gated, inc7 byte-identical) → re-fly rc1 (success = reach@gate-4 >0) → re-measure σ_p0 vs 0.08. Report: handoff/inc8-sigmap0-rc1-2026-06-18/REPORT.md.
- **S5 LOOK-AT PORT into the eval (p2-eval-lookat → main 44617ce, 2026-06-18): the binding blocker is NOT look-at omission.** Disambiguated 3 factors: **(1st, DOMINANT) the eval's hardcoded SIMSTART** (the deploy nose-first spawn the policy never trained on) — no seed flies from it w/ or w/o look-at; from training-native **TRAINRESET, seed0 FLIES + reaches gate-4 136/200 (0.68).** **(2nd, binding for gate-4) the look-at PITCH injection breaks the eval plant** (g_pitch=±3 → 0/40): the legacy CTBR body-Y rate-map alias is self-consistent for policy OUTPUT but NOT an INJECTED physical pitch correction (body-Z survives, body-Y doesn't); worker did NOT force a sign (both break). look-at YAW ported faithfully (flies; inc7 byte-id sha256 c705e39d + green_gate OFF==inc7 AST GREEN). **(3rd) generalization:** inc7-on-emul also degraded (4/40 from trainreset); inc8 seeds 1&2 die at gate-1. **Provisional σ_p0 (seed0, trainreset, YAW-ONLY, 200 ep) = 0.177 m (bias −0.095, p99 0.375) → NO-GO vs 0.08** — but PESSIMISTIC (pitch disabled → gate-4 fix-rate≈0; gate-4 is VERTICAL-σ-dominated, pitch = the elevation DoF). 🚩 **NEXT = fix the pitch-axis injection** (the diffaero↔plant_step body-Y rate map — empirical, like the sign footguns; the legacy alias only validated for policy OUTPUT, not injected corrections) → re-measure a faithful gate-4 σ_p0 on trainreset; then RW_TC sweep + MonoRace idea#1 (absolute centring) tighten σ_p0. green_gate 947. Report: handoff/inc8-eval-lookat-2026-06-18/REPORT.md.
- **S5 σ_p0 EVAL — YAW-INJECTION BUG FIXED (p2-eval-pitch → main ec4cb03, 2026-06-18); SUPERSEDES the bullet above (pitch was NEVER the bug).** Matched-state body-rate trace (handoff/inc8-eval-pitch-2026-06-18/trace_bodyrate.py) PROVED: t_cam numpy(eval)==torch(train) Δ=[0,0,0] (kills the commander's "t_cam Y-divergence" leading hypothesis) AND the PITCH (body-Y) injection was ALWAYS bit-faithful to training. **The real bug was YAW (body-Z):** the eval reconstructed the FLU action from `policy_step`'s FRD output via the TRAINING adapter `_FLIP=[1,-1,-1]`, but `policy_step` emits FRD through the LIVE wire map `_ACT_FLU_TO_FRD=[1,-1,1]` (`fly_rl.py:97,544`). The eval plant runs `_RATE_SIGN_LIVE=[1,1,1]` (`offline_rollout.py:64`) — 🚩 **NOT trained-world [1,1,-1]; the prior eval-lookat REPORT asserted [1,1,-1], and THAT error is exactly what made the previous diagnosis (wrongly) blame pitch ("body-Z survives, body-Y doesn't" was backwards).** `_FLIP` and `_ACT_FLU_TO_FRD` agree on roll/pitch, differ on YAW only → the look-at's yaw realized rate was sign-flipped (~0.06–0.10 rad/s) vs training. FIX = invert/re-apply `_ACT_FLU_TO_FRD` at `contact_true_eval.py:365,367` (both maps involutory → look-at OFF/out-of-band ⇒ `rate_frd` unchanged ⇒ inc7 byte-id; inc8 yaw-only DID change [yaw WAS the buggy axis] but flight/σ unchanged: reach ~0.67, σ_p0 0.177). COMMANDER-VERIFIED: all 3 constants read in code; primitive 17/17; green_gate GREEN 947; merge-gate diff clean (does NOT touch the policy-output mapping or the `_rate_sign` alias — escape hatch respected). 🚩 **2-axis STILL 0/200 all seeds — NOT the injection (now faithful all-axes): the #37 obs-fidelity gap (torch-train emul OBS ≠ numpy-eval emul OBS), amplified by the aggressive g_pitch=3 look-at** (monotonic in g_pitch; both signs; dies with velocity too; a low-noise KF crashes even yaw-only). rc1 flies 2-axis in TRAINING but not on the numpy-eval obs. **Best-available FAITHFUL σ_p0 = yaw-only seed0 0.177 m (bias −0.095, p99 0.375) → NO-GO vs 0.08, pessimistic** (no elevation pointing → gate-4 fix-rate≈0). 🚩 **NEXT = measure 2-axis σ_p0 where rc1 PROVABLY flies — instrument the GT gate-4 crossing IN THE TORCH TRAINING env (rc1 flies 2-axis there with the correct per-episode DR spread), OR on the de-risked spike production chain (obs Δ=9e-9) — NOT the numpy-emul.** Then RW_TC sweep / MonoRace idea#1 (absolute centring) tighten; seed-1/2 generalization + real-8-kpt accuracy (best.pt unstaged) remain. Report: handoff/inc8-eval-pitch-2026-06-18/REPORT.md.
- 🚩 **§inc8-2026-06-19 — DETERMINISTIC-STABILITY RETRAIN + THE BAR CORRECTION (supersedes the NO-GO ending above).** **(A) Det-stability retrain** (noise-anneal/std-cap lever, MERGED 42ddb0b, green_gate GREEN): the FIRST deterministically-flyable 2-axis inc8 (gp1.0 DETERMINISTIC reach 0.467; rc1 was 0.000 — crashed on its mean). ROOT CAUSE of rc1 (measured, handoff probe_logstd): 2 `actor_logstd` channels SATURATED at std=exp(2)=7.39 (entropy bonus, NO std ceiling) → flew on noise, mean unflyable. FIX = clamp actor_logstd to a std CEILING (hold ≈0.6 = the flight win; the aggressive anneal→0.03 DEGRADES det reach) + SELECT on DETERMINISTIC reach, not the stochastic high-water "best". A REUSABLE lever for any deployable inc8 (rl/inc8_noise_anneal.py + inc8_snapshots.py). **(B) THE BAR CORRECTION (511e85c, Fengyou caught a real error):** the σ_p0 ≲0.08 bar was a `margin_envelope.py` DOUBLE-COUNT (drone subtracted twice: 0.215 chassis in W_EFF AND r as the contact halo; 0.08 = 0.235/3). The SIM is correct (pass = offset < 0.75−r → real clearance 0.37–0.47 m); REAL bar = **σ_p0_lat ≲0.15 (p99 ≲0.45)**. Measured σ_p0 0.15–0.20 / p99 0.37–0.40 = **MARGINAL-PASSING (sim success ~0.65), NOT NO-GO.** ⇒ gate-4 = a **REACH/PASS-RATE control problem** (gate off the racing line; clearance is the SAME as every gate), NOT sub-cm centering. 🚩 **RL IS the right tool — the "pivot off RL to a near-field-gate estimator" recommendation is RETRACTED; next lever = RL to lift reach/pass-rate; vision = SUPPORT, not the closer.** Live σ_p0 instruments (inc8_sigmap0_eval.py / inc8_sigmap0_torch_eval.py) σ_target default corrected 0.08→0.15 (49b1aff). Full: handoff/inc8-deterministic-retrain-2026-06-19/REPORT.md (the report's own NO-GO/pivot framing is itself banner-superseded). [A later reach/pass-rate re-train result is HELD/stashed pending Fengyou — not banked here.]
