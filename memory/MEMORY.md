...
**🚩 GATE CONTACT = INVALID RUN.** CORNER-PASS bound = 4.27 s (inscribed-circle, contact-free); 4.13 s = INVALID. Detail [[project-phase2-rl-vision-decisions]] §CORNER-PASS.
...
**🚩 STYLE ENVELOPE COST:** ≤65° roll costs ~2.3 s/lap. Relaxation ladder: (1) rw_tilt 96→48, (2) free-cone 60°→75–80°, (3) unconstrained (~6.6 s). UNBLOCKED (inc7 live confirmed). **🚩 PRIMARY MARGIN GUARD = GATE-4** (VERIFIED binding, SIMSTART, 0.155 m @ r=0.38; gate-4 registration-confirmed offline — live-crosses ~0.10 m in-plane, rules out ≥0.5 m offset). Gate-5 CLEAN (0.314 m). Relaxation must NOT drive gate-4 margin negative. Detail [[project-phase2-rl-vision-decisions]] §INC5 + §TILT-CONCENTRATION.
...
② ✅ **SHADOWPC-INC7-LIVE DONE** — 5/5 standing + 2/2 bridge FINISHED. Finding-A FALSIFIED (see READ FIRST). Bimodal CLASSIFIED (see READ FIRST). Residual LOW: per-tick gate-transition debug_obs (fold into next ShadowPC touch).
③ ✅ VISION-FRAME-FIX DONE (8d7b0b3).
④ Envelope ladder step 1 (rw_tilt 96→48, UNBLOCKED) — user selects thrust.
⑤ S19 mixer contradiction; inc6 = historical fallback.
⑥ 🆕 **INC8 BINDING GATE = GATE-4** (SIMSTART, radius-invariant; 0.155 m @ r=0.38, registration-confirmed). Gate-5 CLEAN (0.314 m). Measurement bug fixed (82c2d20). Binding START-TYPE-DEPENDENT: gate-4 SIMSTART only; trainreset MIS-RANKS. **WINNER-VALIDATION RIDER:** fresh-reset live batch ≥3–5 laps before crowning. Detail [[rl-increment-history]] §INC8-DESIGN + §Phase-0(b).
...
- **RL = COMMITTED VQ2 path.** Stage verdicts [[project-phase2-rl-vision-decisions]]; lineage [[rl-increment-history]].
  ...
  - **S2 architecture pending:** decomposed plan-line + RL-tracker vs monolithic. Gap data: 35.3 s → 8.3 s twin → 4.27 s TOGT; style cost 2.3 s/lap.
...
**🆕 ADVISOR-TRIAGE OPEN QUEUE:** ① photoreal detector (Blender/Cycles next); ② ONNX/TRT latency; ④ case-C noise model; ⑤⑥ SHADOWPC-VISION-CAL (in-loop latency L + 2-corner PnP + Bayesian-IoU extrinsic + per-gate last-fix distance). Estimator K+L+N PLANNED. REJECTED: HSV pre-filter. Detail [[project-phase2-rl-vision-decisions]] §CASE-C-READINESS + §ADVISOR-TRIAGE-2026-06-11.
