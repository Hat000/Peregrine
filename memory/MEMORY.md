...
**🆕 NEXT queue (post-inc7-live-confirm + S2-DECIDED 2026-06-13):**
① ✅ **INC7 LIVE CONFIRMED** — `stage1_inc7_actor.pth` s0, md5 2AFF8D62569BA5FEC769028D72AF50E3, sidecar {act_max_thrust 3.765, act_max_rate 3.14}. sr=1.000 / t_med=9.76 s / gen=0.924 / tails max 0.169 m / gate-3 barrier GONE. Deployment ~11.45 s. **🚩 Budget MORE seeds for inc8-class retrains** (2/3 viable; narrow basin). Detail [[rl-increment-history]] §INC7-LIVE-CONFIRMED.
② ✅ **SHADOWPC-INC7-LIVE DONE** — 5/5 standing + 2/2 bridge FINISHED. Finding-A FALSIFIED. Bimodal CLASSIFIED. Residual LOW: per-tick gate-transition debug_obs (fold into next ShadowPC touch).
③ ✅ VISION-FRAME-FIX DONE (8d7b0b3).
④ 🆕 **S2 ARCHITECTURE DECIDED (HIGH confidence, 2026-06-13): `staged_monolithic_then_decomposed`.** inc8 = retrain MONOLITHIC on corrected aero, hybrid-cast (arc-length progress over REBUILT contact-safe line), open cone one rung. Decomposed = triggered fallback ONLY (trigger: toy-MPCC k<~1.25 AND monolith starts corner-cutting at gate-4). Detail [[project-phase2-rl-vision-decisions]] §S2-DECISION. **Sub-tasks (inc8 ordered plan):** (1) graft arc-length progress reward; (2) build corrected-aero min-snap+TOPP line generator; (3) rw_tilt 96→48 then free-cone 60°→~70° GATED on gate-4 metric; (4) retrain ≥4 seeds; (5) scipy-SLSQP toy MPCC probe (decomposed fallback decision gate).
⑤ S19 mixer contradiction; inc6 = historical fallback.
⑥ 🆕 **INC8 BINDING GATE = GATE-4** (SIMSTART, radius-invariant; 0.155 m @ r=0.38, registration-confirmed). Gate-5 CLEAN (0.314 m). Binding START-TYPE-DEPENDENT: gate-4 SIMSTART only; trainreset MIS-RANKS. **WINNER-VALIDATION RIDER:** fresh-reset live batch ≥3–5 laps before crowning. Detail [[rl-increment-history]] §INC8-DESIGN + §Phase-0(b).
⑦ 🚩 **SEND ORGANIZER EMAIL NOW (CRITICAL PATH):** Q① (VQ2 streams pose? — gates whether estimator is the binding VQ2 risk) + Q⑤ (eval-HW GPU? — gates whether CPU-class vision latency flips case-C verdict). These two answers determine the entire ESTIMATOR vs PLANNER priority ordering. Detail [[project-phase2-rl-vision-decisions]] §PLANNING-TOGT-S2.
...
**🆕 ADVISOR-TRIAGE OPEN QUEUE:** 🚩 **ESTIMATOR = CRITICAL PATH** (converged ②③): ULTRACODE-ESTIMATOR-RACESPEED dispatched (test <0.05 m 1-sigma at 37 m/s). ① photoreal detector (Blender/Cycles next); ② ONNX/TRT latency; ④ case-C noise model; ⑤⑥ SHADOWPC-VISION-CAL (in-loop latency L + 2-corner PnP + Bayesian-IoU extrinsic + per-gate last-fix distance). **Organizer Q①+Q⑤ = SEND NOW (gates estimator risk + eval-HW verdict).** Estimator K+L+N PLANNED. REJECTED: HSV pre-filter. Detail [[project-phase2-rl-vision-decisions]] §CASE-C-READINESS + §PLANNING-TOGT-S2 + §ADVISOR-TRIAGE-2026-06-11.
