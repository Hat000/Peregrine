...
⑧ **INC8 SPEC (gate-relative BLUEPRINT.md, 2026-06-13 — Adroit-ready; SHADOWVISION-updated):**
  ...
  - DR (estimator, on obs only): **per-fix lateral σ = U[0.05,0.15] m/axis (MEASURED 0.10 m; supersedes modeled 0.265 m)**; **one-signed PnP/extrinsic bias U[0,0.19] m with per-episode constant RANDOM SIGN** (SUPERSEDES ±0.10 zero-mean — cannot cover a one-signed 0.19 m bias). Map bias NOT injected (drops out).
  - Reward: R1' arc-length over REBUILT corrected-aero reference line (reference_line_vq1.json drag-infeasible+170° inverted → MUST rebuild); T4 finish-time KEPT; R4' **fixed relaxed cone = L0 default** (tilt_free_rad is scalar; confidence-gated cone is portfolio ablation); **🆕 R5' camera-pointing / gate-in-FoV reward = NEW TOP LEVER** — reward keeping active gate centre in-image (de-crab; drives fix-density, the new binding variable for gate-4 margin closure). This is the primary reward change vs original spec.
