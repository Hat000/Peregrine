...
⑧ **INC8 SPEC (gate-relative BLUEPRINT.md, 2026-06-13 — Adroit-ready; SHADOWVISION-updated):**
  ...
  - **INC8 BINDING GATE = GATE-4 (SIMSTART, p90 AND p99, COLD design point).** Report as function of r ∈ {0.21,0.26,0.30,0.33,0.38}; central = r=0.30 (budget 0.235); select at r=0.38 worst-case (budget 0.155). **Primary lever = camera pointing/fix-density (policy-addressable via R5' reward). Secondary lever = ESKF attitude-bias estimation.** If live L3 pins effective bias ≤ ~0.6°, margin CLOSES at r=0.30 cold.
...
- **✅ FIX-SURROGATE DONE (infra, 2026-06-14; branch claude/stupefied-fermi-20126c, NOT merged):** analytic NO-RENDER fix model `rl/fix_surrogate.py` for inc8 training. geometry→p_accept/fix_sigma/sample_fix; calibrated to Track-3 (accept @ peak 0.826, σ_lat 0.104); NOT overfit (LOFO CV). Reward NOT decided (Fengyou owns). σ single swappable checkpoint for recalibration from L3 at-speed recording. → [[index-vision-estimator]] §FIX-SURROGATE
- **🚩 2-AXIS CAMERA-POINTING (2026-06-14):** elevation co-binds — decrab alone insufficient (el-ok 0.10 after decrab; |el|~45° vs VFoV 29°); ×8.5 fix-density gain from 2-axis centering. Mount-uptilt sweep (using fix_surrogate) teed up = top pointing analysis. R5' reward spec is now 2-AXIS. → [[index-vision-estimator]] §2-AXIS CAMERA-POINTING FINDING
