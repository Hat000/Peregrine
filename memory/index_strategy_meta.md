...
## SHADOWPC-CONNECTOR (rev 1, 2026-06-27) — the plan to retire the last copy-paste
- 🚩 **Under the rev-2 COMMANDER MODEL ([[MEMORY]]), the prompt-emission/paste directives above now apply ONLY to ShadowPC relays.** Laptop work = background Agents/Workflow (no paste); Adroit = adroit-connector driven DIRECTLY from the commander session.
- **adroit-connector** (`C:\Users\Fengy\Downloads\Projects\Adroit\adroit-connector\adroit.py`, ~21 KB paramiko, its OWN repo + `.venv`) is already host-parameterized (`SSH_HOST = os.environ.get('ADROIT_HOST', 'adroit.princeton.edu')`); subcommands `run`/`x`/`upload`/`shell`/`serve`/`submit`/`jobs`/`tb`-tunnel + `pull_file.py`; the `serve` daemon = ONE Duo approval covers every later `x`; pins `hmac-sha2-256` MAC (raw scp dies on `Corrupted MAC` over the campus VPN). The CORE (connect/run/upload/serve/x/tunnel) is HOST-AGNOSTIC — only `submit`/`jobs` are SLURM-specific.
- **shadowpc-connector = clone adroit.py, 3 deltas:** (1) **reachability** — ShadowPC is a NAT'd cloud Windows box with no inbound port → **Tailscale/ZeroTier** mesh (stable private IP, survives reboot; ~15 min) or a reverse tunnel; (2) point `SSH_HOST` at the ShadowPC mesh IP + a ShadowPC key, drop `submit`/`jobs`; (3) **interactive-desktop bridge for the GUI sim** — a vanilla Windows SSH `exec` lands in a NON-interactive session with no desktop → it CANNOT focus windows / key-send / render, which `simops_helper.py` (ctypes) needs. Fix: run the command in the console session via `schtasks`/PsExec `-i`, or have SSH merely TRIGGER a local ctypes agent. 🚩 **Non-GUI ShadowPC work (file transfer, offline analysis, vision training, image render) works IMMEDIATELY like Adroit; only the live-sim *drive* needs the bridge.**
- Connector-side hygiene: `adroit-connector/command_history.log` had ballooned to **110 MB** (seen 2026-06-27) — truncate it.
