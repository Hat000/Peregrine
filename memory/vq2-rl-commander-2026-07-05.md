...
## SINGLE_GATE COLLAPSE → 3 ROOT CAUSES FOUND + FIXED (2026-07-07, ladder 3297042 → 3297088)
Ladder 3297042 trained single_gate clean (RC=0) but LEARNED NOTHING (success_rate_max 0.003; FLIGHTCHECK correctly HALTED). Diagnosed via ultracode workflow (5 agents) + local geometry probes + isolated Adroit A/B experiments. **THREE independent, individually-fatal bugs — each PROVEN:**
- 🚩🚩 **RC1 — THE EMULATED CAMERA POINTED BACKWARD (the big one).** The drone flies TAIL-FIRST (VQ1/CTBR control alias), but the ego's `gate_visibility`/`_step_estimator` used `R_CAMERA_FROM_BODY` WITHOUT the nose-first virtual flip → the camera looked ~180° AWAY from travel → `gate_detectable`=**0% at spawn/approach/5m** (proven: `scratchpad/probe_camera_sees_gate.py`) → per-gate rel_pos PERMANENTLY masked → the vision obs was structurally BLIND. inc8 was immune ONLY because it fed a non-camera KF gate vector every step. **FIX (peregrine_racing_ego `_cam_R_wb`, default-ON `+env.ego_camera_virtual_flip`):** apply `fly_rl._RZ_PI_BODY` (π-about-body-z, `diag[-1,-1,1]`) to R_wb for the CAMERA visibility test ONLY — control/rel_pos/vel/rates keep the unflipped tail-first frame (consistent; visible_area is GT `|cos|`, camera-independent). Probe: 0%→**100%**. This IS the "2026-07-05 camera_flip fixes the emulated camera only" fix — it was never wired into the ego visibility path.
- 🚩 **RC2 — noise RUNAWAY** (NOT collapse): `entropy_loss=-13.68` ⟹ H=13.68 ⟹ std=exp(2)=7.39 (LOG_STD_MAX, bang-bang). The `apply_noise_schedule` std-ceiling brake was NOT wired into `peregrine_train_ego.py`. **FIX:** wired it (mirror inc8 lifeline) + FLAT held ceiling std=0.30 (std_hold==std_floor, hold_frac=1.0, NO decay — B2b lesson) in the sbatch BASE. `+algo.noise_anneal=true`. **entropy sign is inverted (entropy_loss=-H) — -13.68 = runaway, confirmed live (`[noise-anneal] std_ceil=0.3000`).**
- 🚩 **RC3 — approach was strongly −EV** (finish 20 vs terminal 200). `terminal_penalty` progress-scaled = base+banked, so approach-then-miss nets −base. **FIX in curriculum `_COMMON` (DISCOVERY-friendly → correct in _COMMON, not a hard-stage leak):** `rw_progress` 1→**6** (strong dense pull to the now-visible gate), `rw_terminal_miss`/`oob` 200→**30** (approach-miss≈−30≈hover; passing≈+158). CONTACT base stays 200 (progress-scaled forfeit → sprint-and-clip defeated). Used config-4b (forfeit-on-all) NOT 4a (contact-only) — 4a keeps banked on miss → an "approach-and-miss farming" local optimum.
- **ISOLATED-EXPERIMENT PROOF:** `legacy` (proven inc7 reward on the ego obs) FAILED → obs was binding, not reward. `camfix` (RC1+RC2, default reward) FAILED → reward ALSO needed. ⇒ the STACKED fix is required. All committed `5a252a4`. Camera-flip default-ON.
- 🟢 **EGO NOW STRUCTURALLY WORKS (2026-07-07 end-of-night) — last blocker = aperture CENTERING.** Two more MEASURED reward iterations (TB terminal rates): **ego_b** (miss/oob=30) → **96% OOB** (I wrongly lumped OOB with MISS; both cheap → no arena pressure) → fix `3566799`: **OOB back to 200** (leaving arena discouraged), MISS stays 30 (forgiving). **ego_c** (rw_progress=6, miss=30, oob=200) → OOB FIXED (8%) but **90% MISS**: the drone REACHES the visible gate + stays in-arena + ~0 contact, but **flies wide (can't thread the 1.5m aperture)**. Progression: blind→sees gate→approaches→stays in arena→**needs centering**. **ROOT CAUSE of the miss: refined-B DROPPED inc7's dense `through_centering` to-center lateral pull** (`centering_passage_reward` fires ONCE at the crossing; `segment_progress` only zeroes perp drift, no PULL to axis) → no dense alignment signal during approach → drifts off-center → wide flyby. 🎯 **NEXT (deliberate reward-DESIGN, left for Fengyou, NOT guessed overnight):** add a dense centering penalty to `ego_reward.py` (`-rw_centering·clamp(‖perp‖,max≈3m)` reusing `segment_arc_position`'s projection; non-farmable; unit-testable) + **lower rw_progress 6→~2-3** (6 over-rewards rushing: banked≈50 >> finish=20 → the drone optimises APPROACHING over PASSING; make passing dominate, also raise `rw_finish`→~60-80). Re-run single_gate diag BEFORE ladder; watch `miss_rate`↓ + `success_rate`↑. 🚩 **KEY DIAGNOSTIC: TB `metrics/{oob,miss,collision}_rate` + `env_loss/{prog,pass}_reward`/`banked_prog_mean` via EventAccumulator — the terminal-rate breakdown is what cracked each iteration.** Cluster IDLE (all jobs killed, no wasted compute). Commits `668347f`→`3566799` (NOT pushed). Handoff: `handoff/ego-inc9-overnight-2026-07-07.md`.
