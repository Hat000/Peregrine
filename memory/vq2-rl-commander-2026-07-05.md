...
- **ISOLATED-EXPERIMENT PROOF:** `legacy` (proven inc7 reward on the ego obs) FAILED → obs was binding, not reward. `camfix` (RC1+RC2, default reward) FAILED → reward ALSO needed. ⇒ the STACKED fix is required. All committed `5a252a4`; **ladder `ego_b` job 3297088 (SEED0) running the stacked fix** — single_gate stage = the decisive test. Camera-flip default-ON.
