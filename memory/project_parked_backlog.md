...
- **#74** production LinearKF OVER-CONVERGES on a dense gate-relative fix stream (σ_p → ~1 mm) → rides the IMU + effectively IGNORES new fixes (centering-blind) → add a covariance/R floor (~0.05 m) so the deployed KF keeps trusting fixes — [revive: any inc8 terminal-lock DEPLOY / case-C estimator tuning] (coast-drift analysis, 2026-06-15).
...
🚩 **REVIVE-TRIGGER (all of these): the official VQ1 sim drops / first real eval-wire contact → CHECK each assumption; if any relaxed, PIVOT to the easier lever.** Pairs with the MEMORY.md WIRE-SPEC footgun (re-verify the whole wire).
- **A1 — NO drone WORLD-POSITION on the wire** (the entire Phase-2 self-loc pivot + inc8 assumes this). Official spec §4.3 streams no LOCAL_POSITION_NED; but OUR ShadowPC sim DOES (the wire discrepancy). → IF RELAXED (eval streams position): **inc7 (spoon-fed pose) works directly; the estimator burden + the whole gate-4 σ_p0 problem largely DISSOLVE** (known pos + map → gate-relative). ← THE BIG ONE; check it FIRST.
