"""Attempt-4 pursuit/approach analysis: detector replay (gate visibility) + tlog HIGHRES_IMU realized
attitude (accel tilt + gyro) over the first ~4 s, to confirm the pitch-saturation tumble after a
stable approach. Usage: python analyze_run.py <session_dir> <out_dir>
"""
...
print(f"[tlog] {len(imu)} HIGHRES_IMU samples first 4s")
for s in imu[::max(1,len(imu)//28)]:
