"""Render 'what the telemetry vision saw' for an attempt-5 run: the live VQ2 camera feed with the
red_glow detector's gate overlay (corners + quad) and the PnP range. Slowed ~3.75x (8 fps) + 2x.
    ...
    for o in obs:
        ...
        try:
            ...
            if pose is not None and np.isfinite(pose.range_m):
                ...
                cv2.putText(big, f"{r:.1f}m", (c[0]-20,c[1]), cv2.FONT_HERSHEY_SIMPLEX, 0.6,
