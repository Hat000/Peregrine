        ...
        if session_dir is not None:
            try:
                ...
                _ego_log.append({
                    ...
                    "arrest_id": arrest_id,
                    # D1 (only present when --ego-vel-fuse > 0, so the default log is byte-identical):
                    # the vision-referenced velocity-error tracker state -- bias, the APPLIED
                    # (LOS-perpendicular) correction, window length, update/clip counts.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--ego-vel-fuse", type=float, default=0.0,
                    help="EGO VISION-REFERENCED LATERAL VELOCITY gain (ego path only; D1 2026-07-27). "
                         "obs[0:3] is the deploy KF's velocity, and on this wire that KF has NO position "
                         ...
                         "defaults vel_model='legacy', which SEEDS the obs velocity at truth and pulls it "
                         "15%% back toward TRUE body velocity at every vision fix (:815-819), a ~0.01 m/s "
                         "steady-state error. This closes the gap: the tracked gate is a world-FIXED "
                         "landmark, so (anchor lever rotated by the gyro) minus (current fix) is a "
                         "world-referenced measurement of how far the drone actually moved; differenced "
                         "against the dead-reckoned displacement over the same window it yields the KF's "
                         "velocity error, which is tracked with THIS gain and added back to obs[0:3]. "
                         "ONLY the line-of-sight-PERPENDICULAR part is estimated and applied (the "
                         "perpendicular lever noise is 0.036 m vs 0.089-0.36 m along the range axis), so "
                         "closing speed is left entirely to the KF and the range channel's EMA/bbox/"
                         "propagate contaminants are projected out. 0.0 (DEFAULT) = OFF = byte-identical. "
                         "0.15 is the calibrated value (~4-update / ~2 s averaging, matched to the "
                         "measured ~2 s correlation time of the error). Held through a blackout, then "
                         "decayed back to the raw KF velocity.")
