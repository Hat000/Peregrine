...
def build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--ego-vel-fuse", type=float, default=0.0,
                    help="EGO VISION-REFERENCED LATERAL VELOCITY gain (ego path only; D1 2026-07-27). "
                         ...
                         "strapdown dead reckoning. TRAINING's velocity IS vision-corrected: v1.9/v2.0 "
                         "run stage dual_gate_fullstack_floor_pef16, whose stage dict sets "
                         "ego_faithful=True (the LAUNCHERS never mention it), so ego_vel_model takes "
                         "its faithful default 'kf' and ego_estimator.py:777-806 folds vision POSITION "
                         "fixes into the KF. The gap is a CORRECTED KF vs an UNCORRECTED one, and the "
                         "wire cannot form that fix because it needs a MAP (z_datum = gate_pos_datum - "
                         "R_datum @ fix_body) and the deploy profile is map-free. This supplies the "
                         "map-FREE analog: the tracked gate is a "
