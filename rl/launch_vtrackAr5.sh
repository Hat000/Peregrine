...
#
# BASE: this script + the trackA curriculum stage + the rw_v_cap / rw_perception_next / rw_yaw_dither
# reward knobs all live on the vtrackAr5-base branch (commit 629d2a2, "feat(ego Track A): ... full-lineage
# stage") -- the EXACT code Track-A / m8b / rs0 trained from, byte-matching the Adroit peregrine_repo
# checkout. So (unlike an earlier note) the trackA scaffolding IS in git; syncing rl/*.py to Adroit is a
# straight copy of the same tree.
#
...
# 1. cross_zero_m: the effective 4.0 comes from the trackA stage dict's "rw_cross_zero_m": 4.0
#    (rl/vq2_ego_curriculum.py, the dual_gate_fullstack_floor_pef_trackA stage). EgoRewardWeights.from_cfg
#    reads `rw_cross_zero_m` with PRECEDENCE over `cross_zero_m` (and over the 0.75 dataclass default), so
#    the override below is `++env.rw_cross_zero_m=0.75` (a bare `++env.cross_zero_m=0.75` would be INERT).
#    The stage renders `+env.rw_cross_zero_m=4.0` and EXTRA is appended LAST, so `++env.rw_cross_zero_m=0.75`
#    overrides it to the 0.75 aperture-edge. The trackA stage has NO cross_zero_anneal, so NO
#    ++env.cross_zero_anneal=false is needed (a static 0.75 stands for the whole run).
# 2. finish_time: consumed by the T4 finish term (rw_finish + rw_finish_time * t_left); from_cfg maps it
#    via the `rw_finish_time` key (NOT `finish_time`), so the override is `++env.rw_finish_time=0.25`,
#    included below. Verify it maps to weights.finish_time on the PRECHECK.
# 3. return_norm (PopArt) is DEFERRED -- NOT set here (Stage 1 relies on the critic-only grad-clip). Do
#    not add ++return_norm.
# 4. INERT (NO-OP) TOKENS on this base -- kept because they are the m8b recipe verbatim, but on 629d2a2 they
#    map to NO read site, so Hydra `++` silently adds an unused cfg key (NO crash, NO effect):
#      - ++env.course_lateral_offset_lo / _hi : resolve_course_overrides (peregrine_racing_ego.py) does NOT
#        recognize these keys; the per-env xy-shift they were meant to drive is NOT applied. If lateral
#        course randomization is wanted, it must be wired into resolve_course_overrides first.
#      - ++env.ego_coarse_map_live            : peregrine_racing_ego reads no `ego_coarse_map_live`; the
#        coarse map is the STATIC build_coarse_map()-at-reset map regardless. (The live-map behaviour the
#        recipe intends is not gated by this key on this base.)
#    Every OTHER ++ token below DOES map to a real field/read site (verified against from_cfg + the
#    peregrine_racing_ego getattr sites + the anneal resolvers).
...
#
# PRECHECK ASSERTIONS (the 3-update sbatch smoke is a WIRING check only; run a ~50-100 update smoke with
#    save_freq~=25 to also see BEST_CKPT): the log must show
