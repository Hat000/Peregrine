#!/bin/bash
# =====================================================================================================
# v1.7 = v1.6 (camera mount + range detection + aperture margin + pitch quietness, ALL wire-flown) PLUS
# four v1.7 mechanisms aimed at the FOUR residual failure modes the ~250 v1.6 deploy flights left, and
# warm-started from the DEPLOY-PROVEN v1.6 lead. Every new knob is config-gated and DEFAULT-OFF: a config
# without the v1.7 keys trains BYTE-IDENTICAL to v1.6 (pinned by tests/test_v17_mechanisms.py's
# equivalence + RNG-neutrality tests). NO new curriculum stage -- the stage stays
# dual_gate_fullstack_floor_pef16 and every v1.7 knob rides in on EXTRA (hydra ++ beats the stage dict,
...
#   3.3. Four residual modes, in priority order -- v1.7 attacks 1-3 BY CONSTRUCTION, 4 is the pilot's ask:
...
#
# THE FOUR MECHANISMS
#   M1 HANDOFF SPAWN REALISM ++env.handoff_spawn_frac=0.33 (attacks 1) -- a SIBLING spawn mode. MEASURED
#     ROOT CAUSE (probe of the real projection, re-derived in the tests so it cannot drift):
#         spawn attitude              camera optical axis      detectable gate-elevation band
#         VQ1 tilted pad (-17.8 deg)        +2.0 deg                 -31.0 .. +35.0 deg
#         WIRE handoff (LEVEL)             +19.5 deg                 -13.5 .. +52.5 deg
#     (+19.5 == the baked +20 deg mount; the pad tilt cancels ~all of it.) So at EVERY training spawn the
#     gate sits AT OR ABOVE the optical axis, while at the wire handoff -- level, ~4 m up, gate 10-11 m
#     out at roughly its own height -- it sits ~20 deg BELOW it. The policy reads "gate low in frame" ->
#     "pitch down", has NEVER seen that input from rest, saturates, and overshoots out the frame TOP. The
#     existing velocity constraints (overspeed 12, vcap 7.5) bind only at the END of an acceleration, so a
#     hard dive from rest never touches them -- nothing prices the maneuver that costs gate 0. The fix is
#     DISTRIBUTIONAL: 1/3 of envs spawn LEVEL, AT REST, on the incoming leg, at a sampled SLANT range
#     (9.5-11.5 m == the deploy first-lock band 10.3-11.2 widened) with the gate +-1.5 m of the drone's own
#     altitude. The whole band lands inside the level pose's detectable window, so gate 0 is ACQUIRED AT
#     t=0 on every handoff env -- matching "first slot0 lock at 10.3-11.2 m on EVERY flight".
#     NOT KNOBBED, ALREADY TRUE: hover thrust. DiffAeroDynamics.reset_idx re-seeds _thrust to hover and
#     _sf_body_flu to the level-hover sample on EVERY reset; levelling the pose makes that seed CONSISTENT
#     with the attitude for the first time. NOT REPRODUCED: the wire's absolute ~4 m AGL -- the actor obs
#     is POSITION-FREE, so only RELATIVE geometry is experienceable, and pinning absolute AGL would break
#     the acquisition invariant (elevations to -25 deg == out of frame). Absolute height only buys FLOOR
#     ROOM before a dive turns fatal; that is ++env.handoff_min_agl_m (default 0.5, raise toward ~4 to buy
#     the dive budget too).
...
#     so it WHISPERS. v1.7 instead MULTIPLIES the positive per-step progress credit by a framing factor
...
#
# ARMS (each a SEPARATE SLURM job so one seed's crash cannot kill the others). The v1.7 mechanisms are
...
#   Q  (the v1.6 lead temperament), seeds 0,1 : yaw_dither 0.4 / yaw_duty 0.15 / pitch_duty 0.1. TAG=v17Q_s{0,1}.
#   W  (heavier-damped),           seed 0    : yaw_dither 0.6 / yaw_duty 0.4  / pitch_duty 0.2. TAG=v17W_s0.
#
# ---------------------------------------------------------------------------------------------------
# PHASE 1 (smoke):  bash launch_v17.sh
#     submits ONE smoke (v17smoke_s0, UPD=100, CORE=Q wiring). Prints its jobid + the ADJUDICATION
...
#     RUNDIR=/scratch/network/fl3689/diffaero/outputs/train/ego_dual_gate_fullstack_floor_pef16_seed0_v17smoke_s0
...
#          $RUNDIR/.hydra/config.yaml   # 0.33 / 9.5 / 11.5 / 1.2 / 15.0 / 1.0 / 0.5 / 0.75 (all under env:)
...
#     4. grep EGO_PRECHECK_RC /scratch/network/fl3689/peregrine_vq2_ego_v17smoke_s0.out  # == 0, no NaN
...
# PHASE 2 (arms):  MODE=arms SMOKE_JID=<smoke_jobid> bash launch_v17.sh
...
# =====================================================================================================
set -euo pipefail
STAGE=dual_gate_fullstack_floor_pef16  # REUSED from v1.6 -- every v1.7 knob is a +env. key passed here
...
# --- v1.7 M1: HANDOFF SPAWN REALISM (the wire's from-rest acquisition geometry) ---
HANDOFFFRAC=${HANDOFFFRAC:-0.33}       # handoff_spawn_frac; 0.0 == OFF == byte-identical v1.6
...
# --- v1.7 M2: BLIND-FLIGHT ABORT (the pitch-cap replacement) ---
...
# --- v1.7 M3: PROGRESS FRAMING MULTIPLIER + the (guard-capped) additive perception raise ---
...
# ---- THE v1.7 SHARED MECHANISM block (M1 handoff spawn + M2 blind abort + M3 framing multiplier and the
...
# per-arm EXTRA builder: yaw_dither + yaw_duty + pitch_duty are per-arm (Q vs W); the v1.7 mechanisms are
...
if [ "${MODE}" = "smoke" ]; then
  ...
  submit "v17smoke_s0" 0 "$(extra_for 0 "${YAWDITHER}" "${YAWDUTY}" "${PITCHDUTY}")" 100
  cat <<'EOF'

=== v1.7 SMOKE submitted (UPD=100, CORE=Q wiring: v1.5 + v1.6 CORE + M1 handoff spawn 0.33 + M2 blind
    ...
    absent-hook != inert footgun L16):
      RUNDIR=/scratch/network/fl3689/diffaero/outputs/train/ego_dual_gate_fullstack_floor_pef16_seed0_v17smoke_s0
      1. grep -nE 'handoff_spawn_frac|handoff_range_lo_m|handoff_range_hi_m|blind_abort_s|blind_abort_range_m|rw_progress_frame_mult|progress_frame_floor|pass_margin_final_m' \
           $RUNDIR/.hydra/config.yaml   # expect 0.33 / 9.5 / 11.5 / 1.2 / 15.0 / 1.0 / 0.5 / 0.75 (under env:)
      2. grep -nE 'rw_perception|rw_perception_next|overspeed_abort_mps|rw_progress_vcap_mps|ego_vision_detect_mode|ego_cam_mount_pitch_deg' \
           $RUNDIR/.hydra/config.yaml   # 0.016 / 0.004 / 12.0 / 7.5 / range / 20.0 (v15+v16 carryover)
    ...
    THEN release the real arms (afterok the smoke):
      MODE=arms SMOKE_JID=<smoke_jobid> bash launch_v17.sh
    ESCAPE HATCH (no dependency): MODE=arms SMOKE_JID=none bash launch_v17.sh
EOF
  exit 0
fi
...
for ARM in ${ARMS}; do
  case "${ARM}" in
    Q) for s in ${SEEDS_Q}; do submit "v17Q_s${s}" "${s}" "$(extra_for "${s}" "${YAWDITHER}" "${YAWDUTY}" "${PITCHDUTY}")" "" "${DEP}"; done ;;
    W) for s in ${SEEDS_W}; do submit "v17W_s${s}" "${s}" "$(extra_for "${s}" "0.6" "0.4" "0.2")"                        "" "${DEP}"; done ;;
...
=== v1.7 arms submitted [Q W]. ADJUDICATION PROTOCOL (post-hoc; TAILS, NOT MEDIANS -- one strong event
    ...
    STEP 3 (the v1.7-specific reads -- these decide whether the MECHANISMS worked, not just whether the
      policy is quiet):
