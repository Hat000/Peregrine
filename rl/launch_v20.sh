...
extra_for () {   # $1=SEED  $2=vert_on
  local SEED="$1" VON="${2:-1}"
...
++env.ego_vision_cadence_seed=$((20260725 + SEED)) +init_from=${WARM}"
}

if [ "${MODE}" = "smoke" ]; then
  # -------- PHASE 1: ONE smoke, CORE=V wiring (vertical ON), UPD=100, seed 0 --------
  submit "v20smoke_s0" 0 "$(extra_for 0 "${VERTON}")" 100
  cat <<'EOF'

=== v2.0 SMOKE submitted (UPD=100, CORE=V wiring: v1.9 W roll damping + VERTICAL STRUCTURE
...
for ARM in ${ARMS}; do
  case "${ARM}" in
    V) for s in ${SEEDS_V}; do submit "v20V_s${s}" "${s}" "$(extra_for "${s}" 1)" "" "${DEP}"; done ;;
    C) for s in ${SEEDS_C}; do submit "v20C_s${s}" "${s}" "$(extra_for "${s}" 0)" "" "${DEP}"; done ;;
    *) echo "!! unknown ARM '${ARM}' (expected V / C) -- skipping" ;;
  esac
done

cat <<'EOF'

=== v2.0 arms submitted [V C]. ADJUDICATION PROTOCOL (post-hoc; TAILS, NOT MEDIANS -- one strong event
