...
def test_profile_wiring_a30():
    ...
    assert ov["total_accel_cap_mps2"] == 4.0          # turn iteration (231155): 3.0->4.0 (22 deg bank; more roll)
    assert ov["pursuit_yaw_slew_rps"] == 0.9          # A36 coordinated-turn: yaw 1.5 -> 0.9 (roll-led); FROZEN in the turn iteration
    ...
    assert prof.nav_config.vertical_estimator_overrides == {"use_zoff_filter": True,
                                                            ...
                                                            "use_zoff_big_trust": True,  # A35 Fix-2
                                                            "zoff_prop_bound_m": 3.5}   # R2-2 B1
...
def test_make_seeker_threads_a30_for_vq2_case_c_only():
    ...
    assert c.total_accel_cap_mps2 == 4.0    # turn iteration (231155): 3.0 -> 4.0 (more roll; cap was railing)
