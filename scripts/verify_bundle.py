"""verify_bundle.py — recording-bundle integrity validator (PROMOTED to scripts/ 2026-06-17).

PROMOTION NOTE: this tool previously lived only under ``handoff/simops-mastery-2026-06-13/``
and ``handoff/at-speed-sigma-2026-06-15/analysis/`` (byte-identical copies, stranded off the
canonical path). The burn closed-loop harness (``scripts/diagnose_session.py``) needs it as a
first-class primitive, so it is promoted here UNCHANGED in behaviour. The ONLY edit vs. the
handoff copies is the ROOT anchor: a handoff copy is 2 dirs deep (``handoff/<task>/x.py``) and
used ``parents[2]``; this canonical copy is 1 dir deep (``scripts/x.py``) so it uses
``parents[1]``. Verdict logic, checks, thresholds, and JSON shape are identical.
...
Usage: .venv\\Scripts\\python scripts/verify_bundle.py <run_dir> [--json OUT]
"""
